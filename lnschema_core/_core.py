...
from . import _USE_DJANGO as USE_DJANGO
    ...
    class File(BaseORM, table=True):  # type: ignore
        ...
        def replace(
            ...
            if kwargs["name"] != self.name:
                logger.warning(
                    f"Your new filename '{kwargs['name']}' does not match the previous filename '{self.name}': to update the name, set file.name = '{kwargs['name']}'"
                )
